...
def analyze_user_edits(original_code: str, edited_code: str) -> Dict[str, Any]:
    ...
    change_percentage = ((lines_added + lines_removed) / total_lines) * 100 if total_lines > 0 else 0
...
def generate_behavior_report(behavior_data: Dict[str, Any]) -> str:
    ...
    if total_actions > 0:
        # Most common action
        most_common_action = max(action_types.items(), key=lambda x: x[1]) if action_types else ("none", 0)
