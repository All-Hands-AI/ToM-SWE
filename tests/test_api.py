...
def create_test_app(tom_agent=None):
    ...
    test_app.post("/api/v1/suggest_next_actions")(suggest_next_actions)
    test_app.post("/api/v1/propose_instructions")(propose_instructions)
    test_app.post("/api/v1/send_message")(send_message)
    test_app.get("/api/v1/users/{user_id}/context")(get_user_context)
    
    return test_app


@pytest.fixture
def client_with_mock_agent(mock_tom_agent: AsyncMock):
    """Create a test client with a mocked ToM agent."""
    test_app = create_test_app()
    with patch("tom_swe.api.main.tom_agent", mock_tom_agent):
...
def client_without_agent():
    ...
    test_app = create_test_app()
    with patch("tom_swe.api.main.tom_agent", None):
        yield TestClient(test_app)
...
class TestHealthEndpoint:
    ...
    def test_health_with_agent(self, client_with_mock_agent: TestClient):
    ...
    def test_health_without_agent(self, client_without_agent: TestClient):
...
class TestSuggestNextActionsEndpoint:
    ...
    def test_suggest_next_actions_success(self, client_with_mock_agent: TestClient, mock_tom_agent: AsyncMock):
        ...
        response = client_with_mock_agent.post("/api/v1/suggest_next_actions", json=request_data)
        assert response.status_code == 200
        ...
        mock_tom_agent.analyze_user_context.assert_called_once_with(
            user_id="test_user_123",
            current_query="Debugging Python application"
        )
        mock_tom_agent.suggest_next_actions.assert_called_once()
    
    def test_suggest_next_actions_without_task_context(self, client_with_mock_agent: TestClient):
    ...
    def test_suggest_next_actions_without_agent(self, client_without_agent: TestClient):
        """Test next actions suggestion when ToM agent is not available."""
        request_data = {
            "user_id": "test_user_123",
            "current_task_context": "Debugging"
        }
        
        response = client_without_agent.post("/api/v1/suggest_next_actions", json=request_data)
        assert response.status_code == 503
        assert "ToM Agent is not available" in response.json()["detail"]
    
    def test_suggest_next_actions_invalid_request(self, client_with_mock_agent: TestClient):
        """Test next actions suggestion with invalid request data."""
        request_data = {}  # Missing required user_id
        
        response = client_with_mock_agent.post("/api/v1/suggest_next_actions", json=request_data)
        assert response.status_code == 422  # Validation error
    
    def test_suggest_next_actions_agent_error(self, client_with_mock_agent: TestClient, mock_tom_agent: AsyncMock):
        ...
        assert "Failed to suggest next actions" in response.json()["detail"]


class TestProposeInstructionsEndpoint:
    """Test the propose_instructions endpoint."""
    
    def test_propose_instructions_success(self, client_with_mock_agent: TestClient, mock_tom_agent: AsyncMock):
        ...
        response = client_with_mock_agent.post("/api/v1/propose_instructions", json=request_data)
        assert response.status_code == 200
        ...
        mock_tom_agent.analyze_user_context.assert_called_once_with(
            user_id="test_user_123",
            current_query="Debug the function"
        )
        mock_tom_agent.propose_instructions.assert_called_once()
    
    def test_propose_instructions_without_domain_context(self, client_with_mock_agent: TestClient):
        """Test instruction proposal without domain context."""
        request_data = {
            "user_id": "test_user_123",
            "original_instruction": "Debug the function"
        }
    ...
    def test_propose_instructions_without_agent(self, client_without_agent: TestClient):
        ...
        response = client_without_agent.post("/api/v1/propose_instructions", json=request_data)
        assert response.status_code == 503
        assert "ToM Agent is not available" in response.json()["detail"]
    
    def test_propose_instructions_invalid_request(self, client_with_mock_agent: TestClient):
        """Test instruction proposal with invalid request data."""
        request_data = {
            "user_id": "test_user_123"
            # Missing required original_instruction
        }
...
class TestSendMessageEndpoint:
    ...
    def test_send_message_success(self, client_with_mock_agent: TestClient, mock_tom_agent: AsyncMock):
        """Test successful message sending."""
        request_data = {
            "user_id": "test_user_123",
            "message": "I need help debugging my code",
            "instruction": "Debug the function",
            "current_task": "Debugging Python application",
            "domain_context": "Web development"
        }
        
        response = client_with_mock_agent.post("/api/v1/send_message", json=request_data)
        assert response.status_code == 200
        ...
        assert "user_context" in guidance
        assert "instruction_recommendations" in guidance
        assert "next_action_suggestions" in guidance
        assert "overall_guidance" in guidance
        assert "confidence_score" in guidance
        
        assert guidance["confidence_score"] == 0.85
        assert len(guidance["instruction_recommendations"]) == 1
        assert len(guidance["next_action_suggestions"]) == 2
        
        # Verify mock was called correctly
        mock_tom_agent.get_personalized_guidance.assert_called_once_with(
            user_id="test_user_123",
            instruction="Debug the function",
            current_task="Debugging Python application",
            domain_context="Web development"
    ...
    def test_send_message_minimal_request(self, client_with_mock_agent: TestClient):
        """Test message sending with minimal request data."""
        request_data = {
            "user_id": "test_user_123",
            "message": "I need help"
        }
    ...
    def test_send_message_without_agent(self, client_without_agent: TestClient):
        """Test message sending when ToM agent is not available."""
        request_data = {
            "user_id": "test_user_123",
            "message": "I need help"
        }
        
        response = client_without_agent.post("/api/v1/send_message", json=request_data)
        assert response.status_code == 503
        assert "ToM Agent is not available" in response.json()["detail"]
    
    def test_send_message_invalid_request(self, client_with_mock_agent: TestClient):
...
class TestGetUserContextEndpoint:
    """Test the get_user_context endpoint."""
    
    def test_get_user_context_success(self, client_with_mock_agent: TestClient, mock_tom_agent: AsyncMock):
        """Test successful user context retrieval."""
        response = client_with_mock_agent.get("/api/v1/users/test_user_123/context")
        assert response.status_code == 200
        ...
        # Verify mock was called correctly
        mock_tom_agent.analyze_user_context.assert_called_once_with(user_id="test_user_123")
    
    def test_get_user_context_without_agent(self, client_without_agent: TestClient):
        """Test user context retrieval when ToM agent is not available."""
        response = client_without_agent.get("/api/v1/users/test_user_123/context")
        assert response.status_code == 503
        assert "ToM Agent is not available" in response.json()["detail"]
    
    def test_get_user_context_agent_error(self, client_with_mock_agent: TestClient, mock_tom_agent: AsyncMock):
        """Test user context retrieval when ToM agent raises an error."""
        mock_tom_agent.analyze_user_context.side_effect = Exception("Agent error")
        
        response = client_with_mock_agent.get("/api/v1/users/test_user_123/context")
        assert response.status_code == 500
        assert "Failed to get user context" in response.json()["detail"]


class TestAPIIntegration:
    """Integration tests for the API."""
    
    def test_api_workflow(self, client_with_mock_agent: TestClient):
        ...
        # 2. Get user context
        context_response = client_with_mock_agent.get("/api/v1/users/test_user_123/context")
        assert context_response.status_code == 200
        
        # 3. Suggest next actions
        actions_response = client_with_mock_agent.post(
            "/api/v1/suggest_next_actions",
            json={"user_id": "test_user_123", "current_task_context": "Debugging"}
        )
        assert actions_response.status_code == 200
        
        # 4. Propose instructions
        instructions_response = client_with_mock_agent.post(
            "/api/v1/propose_instructions",
            json={"user_id": "test_user_123", "original_instruction": "Debug the function"}
        )
        assert instructions_response.status_code == 200
        
        # 5. Send comprehensive message
        message_response = client_with_mock_agent.post(
            "/api/v1/send_message",
            json={
                "user_id": "test_user_123",
                "message": "I need help debugging",
                "instruction": "Debug the function",
        ...
        assert all(
            response.json()["success"] for response in [
                context_response, actions_response, instructions_response, message_response
            ]
        )