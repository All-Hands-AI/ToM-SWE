...
"""
    ...
    def update_user_profile_with_session(self, analysis: OverallUserAnalysis) -> None:
        ...
        # Extract expertise indicators from user modeling summaries
        expertise_indicators = []
        for summary in summaries:
            if summary.user_modeling_summary and summary.user_modeling_summary.strip():
                # Extract key phrases that indicate expertise
                modeling_text = summary.user_modeling_summary.lower()
                if any(word in modeling_text for word in ['expert', 'experienced', 'advanced', 'proficient']):
                    expertise_indicators.append(summary.user_modeling_summary)
        
        # Remove duplicates and limit to most recent indicators
        profile.expertise_indicators = list(set(expertise_indicators))[:5]
    ...
    async def process_and_save_single_session(
        ...
        await self.save_session_analyses_to_json(
            user_id, session_id, (analyses, user_messages), session_metadata, base_dir
        )
        ...
        # Step 4: Update overall user profile (Tier 3)
        await self.save_updated_user_profile(user_id, [session_summary], base_dir)
    ...
    async def save_session_analyses_to_jsonl_legacy(
        self,
        user_id: str,
        session_id: str,
        analyses: List[UserMessageAnalysis],
        base_dir: str = "./data/user_model",
    ) -> None:
        """Legacy method for saving session analyses to JSONL format.
        
        This method is kept for backward compatibility with existing tests.
        It delegates to the newer save_session_analyses_to_json method.
        """
        # Convert to the format expected by the new method
        user_messages = [f"Message {i}" for i in range(len(analyses))]
        await self.save_session_analyses_to_json(
            user_id, session_id, (analyses, user_messages), None, base_dir
        )
    ...
    async def process_all_user_sessions(
        ...
        """Process all sessions for a user and update their profile.
        
        This method processes all sessions for a user and updates their overall profile.
        """
        ...
        print(f"Processing {len(session_ids)} sessions for user {user_id}")
        
        session_summaries = []
        for session_id in session_ids:
            try:
                # Process session without updating overall profile (to avoid duplicates)
                result = await self.analyze_all_session_messages(user_id, session_id)
                if not result:
                    print(f"No analyses found for session {session_id}")
                    continue
                ...
                if session_summary:
                    ...
                    print(f"❌ Failed to create summary for session {session_id}")
                    
            except Exception as e:
                print(f"❌ Error processing session {session_id}: {e}")
                continue
        
        # Update overall user profile with all sessions at once (Tier 3)
        if session_summaries:
            await self.save_updated_user_profile(user_id, session_summaries, base_dir)
                
        print(f"✅ Completed processing all sessions for user {user_id}")
